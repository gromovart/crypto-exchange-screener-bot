// internal/core/domain/signals/detectors/continuous_analyzer/calculator/growth_calculator.go
package calculator
...
// GrowthCalculator калькулятор непрерывных последовательностей роста
type GrowthCalculator struct {
	config         common.AnalyzerConfig
	confidenceCalc *ConfidenceCalculator
}

// NewGrowthCalculator создает новый калькулятор роста
func NewGrowthCalculator(config common.AnalyzerConfig) *GrowthCalculator {
...
// Calculate вычисляет сигналы непрерывного роста
func (c *GrowthCalculator) Calculate(data []redis_storage.PriceData, minPoints int) []analysis.Signal {
	...
	for i := 0; i <= len(data)-minPoints; i++ {
		...
		if continuous {
			...
			signal := c.createSignal(symbol, "up", totalChangePercent, minPoints, i, i+minPoints-1)
...
// createSignal создает сигнал непрерывного роста
func (c *GrowthCalculator) createSignal(symbol, direction string, change float64, points, startIdx, endIdx int) analysis.Signal {
	confidence := c.confidenceCalc.Calculate(points, change)
	...
	return analysis.Signal{
		...
		Metadata: analysis.Metadata{
			...
			Tags:           []string{"continuous", direction, "growth", "sequence"},
			IsContinuous:   true,
...
func (c *GrowthCalculator) getMaxGapRatio() float64 {
...
func (c *GrowthCalculator) calculateGap(prev, curr float64) float64 {
	if prev == 0 {
		return 0
	}
...
func (c *GrowthCalculator) UpdateConfig(config common.AnalyzerConfig) {
	c.config = config
	c.confidenceCalc.UpdateConfig(config)
}

// GetName возвращает имя калькулятора
func (c *GrowthCalculator) GetName() string {
	return "growth_calculator"
}
