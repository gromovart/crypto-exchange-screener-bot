// internal/delivery/max/bot/handlers/callbacks/signal_toggle_fall/handler.go
...
// Handler — обработчик переключения сигналов падения
type Handler struct {
	*base.BaseHandler
...
func New(svc signalSvc.Service) handlers.Handler {
	return &Handler{
		BaseHandler: base.New("signal_toggle_fall", kb.CbSignalToggleFall, handlers.TypeCallback),
		service:     svc,
	}
}
...
func (h *Handler) Execute(params handlers.HandlerParams) (handlers.HandlerResult, error) {
	...
	msg := fmt.Sprintf("📉 *Сигналы падения*\n\n%s", result.Message)
