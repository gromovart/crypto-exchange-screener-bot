...
import (
	...
	watchlist_disable_handler "crypto-exchange-screener-bot/internal/delivery/telegram/app/bot/handlers/callbacks/watchlist_disable"
	watchlist_view_handler "crypto-exchange-screener-bot/internal/delivery/telegram/app/bot/handlers/callbacks/watchlist_view"
...
func InitHandlerFactory(
	...
	if services.watchlistService != nil {
		...
		factory.RegisterHandlerCreator(constants.CallbackWatchlistView, func() handlers.Handler {
			handler := watchlist_view_handler.NewHandler(services.watchlistService)
			if subscriptionMiddleware != nil {
				return subscriptionMiddleware.RequireSubscription(handler)
			}
			return handler
		...
		factory.RegisterHandlerCreator(constants.CallbackWatchlistDisable, func() handlers.Handler {
			handler := watchlist_disable_handler.NewHandler(services.watchlistService)
			if subscriptionMiddleware != nil {
				return subscriptionMiddleware.RequireSubscription(handler)
			}
			return handler
