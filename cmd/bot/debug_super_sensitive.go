...
func main() {
	fmt.Println("🚀 ЗАПУСК СУПЕР-ЧУВСТВИТЕЛЬНОЙ ОТЛАДКИ")
	fmt.Println(strings.Repeat("=", 70))
	fmt.Println("⚡ ЭКСТРЕМАЛЬНЫЕ НАСТРОЙКИ: пороги 0.01%, уверенность 1%")
	fmt.Println(strings.Repeat("=", 70))
	...
	fmt.Println("\n" + strings.Repeat("=", 70))
	fmt.Println("🏃 ЗАПУЩЕНО! Ожидание сигналов...")
	fmt.Println("   Пороги настолько низкие, что должны обнаружить ЛЮБОЕ движение")
	fmt.Println("   Даже 0.01% изменение цены будет обнаружено!")
	fmt.Println(strings.Repeat("=", 70))
	...
	go func() {
		...
		fmt.Println("\n" + strings.Repeat("⚡", 30))
		fmt.Println("СУПЕР-ЧУВСТВИТЕЛЬНЫЙ ТЕСТ")
		...
		if totalSignals == 0 {
			fmt.Println("   ⚠️  НОЛЬ СИГНАЛОВ ДАЖЕ С ПОРОГОМ 0.01%!")
			fmt.Println("   🚨 СИСТЕМА НЕ РАБОТАЕТ ПРАВИЛЬНО!")
			fmt.Println("   🔧 Проверьте:")
			fmt.Println("      - API подключение")
			fmt.Println("      - Данные в хранилище")
			fmt.Println("      - Работу анализаторов")
		} else {
			fmt.Println("   ✅ Система работает! Обнаружены сигналы")
