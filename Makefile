...
## db-tunnel-info: Показать полную информацию о подключении
db-tunnel-info:
	@echo "📋 ПОЛНАЯ ИНФОРМАЦИЯ О ПОДКЛЮЧЕНИИ"
	@echo "=================================="
	@./deploy/scripts/db-connect.sh info

## db-tunnel-datagrip: Показать конфигурацию для DataGrip (без SSH)
db-tunnel-datagrip:
	@echo "⚙️  КОНФИГУРАЦИЯ ДЛЯ DATAGRIP (БЕЗ SSH)"
	@echo "======================================"
	@./deploy/scripts/db-connect.sh datagrip

## db-tunnel-config: Показать конфигурацию для DataGrip (со встроенным SSH)
db-tunnel-config:
	@echo "⚙️  КОНФИГУРАЦИЯ ДЛЯ DATAGRIP (СО SSH)"
	@echo "======================================"
...
## db-tunnel-psql: Запустить psql через туннель
db-tunnel-psql:
	@echo "🐘 ЗАПУСК PSQL ЧЕРЕЗ SSH ТУННЕЛЬ"
	@echo "================================"
	@./deploy/scripts/db-connect.sh psql
...
db-connect: db-tunnel-start db-tunnel-test
	...
	@echo "👉 Используйте 'make db-tunnel-datagrip' для конфигурации DataGrip"
